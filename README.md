...
이 차이가 실무에서 중요한 이유가 있다. **파라미터를 하나 틀린 출력은 앱이 받아서 보정할 수 있지만, JSON이 깨진 출력은 받을 수조차 없다.** 그래서 정확도를 한 숫자로 뭉개지 않고 [오답을 7유형으로 분해](#평가--왜-7분류인가)했다.
...
전체 17개는 [context-notes.md](context-notes.md) §3에 근거와 함께 있다. 여기서는 결과에 직접 영향을 준 것만 적는다.
...
| 학습곡선 (250 / 1,000 / 2,000건) | 몇 건부터 효과가 나는가 |
