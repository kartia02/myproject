...
입력 토큰     2,094 → 27         모델 크기  2.3GB (4비트 GGUF)
...
| | GPT-4.1 | base + few-shot | **파인튜닝** | 파인튜닝 (Ollama) | **CPU-only** |
| --- | --- | --- | --- | --- | --- |
| 입력 토큰/건 | 1,542 | 2,094 | **27** | 31 | 31 |
| 지연 p50 | 726ms | 3,800ms | 3,377ms | 5,461ms | **6,812ms** |
| 1,000건당 비용 | $1.037 | $0.361 | **$0.328** | $0.533 | — |

마지막 열이 **GPU를 아예 쓰지 않고 잰 것**이다. 비용을 비우고 지연만 적은 이유가 있다 — (a) 기준의 계산법이 *"로컬 추론 시간 × 클라우드 **GPU** 시간 단가"* 로 착수 때 고정됐고, CPU 시간 단가는 정하지 않았다. **지금 하나 만들어 넣으면 사후 변경이 된다.**

![비용과 지연](assets/cost-latency.svg)

**한쪽은 이겼고 한쪽은 졌다.** 비용은 3.2배 싸지만 지연은 4.7배 느리다. 입력 토큰을 **78배**(같은 모델 기준 `2,094 → 27`) 줄이고도 지연이 늘어난 이유는 [따로 적었다](#결정-2는-비용을-줄였지-지연을-줄이지-않았다).
...
5. **부수 효과 — 입력이 짧아진다** — 툴 설명을 안 넣으므로 입력이 발화 한 줄뿐이다. **2,094 → 27 토큰** (같은 모델이 같은 토크나이저로 센 값)
...
온디바이스를 주장하려면 **GPU 없이 도는 것**을 보여야 한다. GPU에 한 층도 올리지 않고 169건을 다시 쟀다.

| | 정확도 | 지연 p50 | p95 |
| --- | --- | --- | --- |
| Ollama (GPU 64% + CPU 36%) | 79.3% | 5,461ms | 6,627ms |
| **CPU-only (GPU 0%)** | **78.7%** | **6,812ms** | 9,167ms |

**정확도가 사실상 같다 — 169건 중 1건 차이다.** 당연한 결과이고, 그것을 확인하는 것이 목적이었다. 정확도는 **양자화의 성질**이지 계산을 어디서 하느냐의 문제가 아니다. 여기서 크게 벌어졌다면 서빙 경로 어딘가가 잘못됐다는 뜻이었을 것이다.

**지연은 25%만 늘었다.** GPU를 통째로 뺐는데 이 정도인 이유는 **원래도 GPU가 64%만 맡고 있었기 때문**이다 — VRAM 4GB짜리 GTX 970에 3.8GB 모델이 다 안 올라간다. 이 측정에서 **GPU가 실제로 기여하던 몫이 얼마인지가 드러난다.**

> **6.8초는 실용 속도가 아니다.** 말하고 7초를 기다리는 기록 앱은 쓰이지 않는다.
> 다만 이 수치는 **2014년 데스크탑 CPU**의 것이고, 폰의 NPU·ARM 칩은 4비트 소형 모델을 겨냥해 설계돼 성격이 다르다. **그래서 이것은 "폰에서 몇 초"의 답이 아니라 "GPU 없이도 돈다"의 증거로만 쓴다.** 실기기 측정은 [범위 밖](#측정하지-않은-것)이다.
...
```
입력 토큰   2,094 → 27      (78배 감소 · 같은 모델 기준)
지연 p50      726ms → 3,377ms   (GPT-4.1 이 4.7배 빠르다)
```

**입력 토큰은 같은 모델끼리 비교해야 한다.** 토크나이저가 다르면 같은 프롬프트도 개수가 달라진다 — 위 표의 툴 스펙 프롬프트를 GPT-4.1은 1,542개로, Qwen3는 2,094개로 센다. 그래서 **툴 스펙을 뺀 효과는 `2,094 → 27`** 이고, GPT-4.1의 1,542는 비용을 각자 요금 체계로 계산할 때만 쓴다.
...
| | 왜 |
| --- | --- |
| **실기기(폰) 추론** | 범위 밖. **GGUF 2.3GB + CPU-only 지연 p50 6.8초**로 간접 논증했다 — [아래](#cpu만으로도-돈다--다만-느리다) 참조. 폰의 ARM 칩은 이 데스크탑 CPU와 성격이 달라 숫자를 그대로 옮겨 읽으면 안 된다 |
...
# 5. CPU-only 지연 — GPU에 한 층도 올리지 않는다
...
| 지연 측정 | **GTX 970 · VRAM 4GB · 36% CPU 오프로드** · CPU-only 는 `--num-gpu 0` |
