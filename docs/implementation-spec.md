...
현재 안전한 fallback은 Luna를 호출하기 전에 동일한 Python 분석 함수로 변화와 Evidence를 준비한다. 실제 Agent가 호출하는 Tool도 같은 분석 함수를 사용한다. 향후 중복 실행이 확인되면 한 번 계산한 조사 컨텍스트를 Agent와 fallback이 공유하도록 최적화한다.
...
| 검증 계층 | 현재 존재하지 않는 Evidence ID 차단. 수치·인과 표현의 자동 검증은 Evaluation 보강 단계에서 추가 |
